...
const MobileListItem = ({ product, onCompareChange }) => {
  const { formatPrice, addToCartAuto, toggleWishlist, isWishlisted } = useContext(ShopContext)
  const [imgOk, setImgOk] = useState(true)

  const specs = product?.specs || {}
  const offers = product?.offers || {}
  const assured = Boolean(offers.assured)

  const highlights = useMemo(() => {
    const items = []
    if (specs.ramGb && specs.storageGb) items.push(`${specs.ramGb} GB RAM | ${specs.storageGb} GB ROM`)
    if (specs.displayInches)
      items.push(
        `${(specs.displayInches * 2.54).toFixed(2)} cm (${Number(specs.displayInches).toFixed(2)} inch) Display`
      )
    if (specs.rearCameraMp && specs.frontCameraMp)
      items.push(`${specs.rearCameraMp}MP Rear Camera | ${specs.frontCameraMp}MP Front Camera`)
    if (specs.batteryMah) items.push(`${specs.batteryMah} mAh Battery`)
    if (specs.processor) items.push(`${specs.processor} Processor`)
    if (specs.warrantyMonths) items.push(`Warranty: ${specs.warrantyMonths} Months`)
  ...
  const mrp = Number(product?.mrp || offers.mrp || 0)
  const price = Number(product?.price || 0)
  const percentOff = offers.discountPercent
    ? Number(offers.discountPercent)
    : mrp > 0
      ? Math.max(1, Math.min(60, Math.round(((mrp - price) / mrp) * 100)))
      : 0

  const exchangeOff = offers.exchangeOff ? Number(offers.exchangeOff) : 0
  const bankOff = offers.bankOff ? Number(offers.bankOff) : 0
        ...
        <div className='flex gap-3 sm:w-[260px]'>
          <div className='mt-1 hidden sm:block'>
            <input
              ...
              onChange={(e) => onCompareChange?.(product?._id, e.target.checked)}
            />
          </div>
            ...
            <div className='flex items-start justify-between gap-4'>
              ...
              <div className='hidden sm:flex items-center gap-3'>
                ...
                <div className='text-sm text-slate-600'>
                  {reviews.toLocaleString('en-IN')} Ratings &amp; {Math.round(reviews * 0.17 + 20).toLocaleString('en-IN')}{' '}
                  Reviews
                </div>
              </div>
            </div>

            <ul className='space-y-1'>{highlights.map((h) => bullet(h))}</ul>
          ...
          <div className='flex flex-col gap-2 rounded-2xl bg-slate-50 p-4 ring-1 ring-black/5'>
            ...
            {exchangeOff > 0 ? (
              <div className='text-sm text-slate-700'>
                Upto <span className='font-semibold text-slate-900'>{formatPrice(exchangeOff)}</span> Off on Exchange
              </div>
            ) : null}
            {bankOff > 0 ? (
              <div className='text-sm font-semibold text-emerald-700'>Bank Offer: {formatPrice(bankOff)} off</div>
            ) : null}
