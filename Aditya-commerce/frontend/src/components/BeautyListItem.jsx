...
const BeautyListItem = ({ product }) => {
  ...
  const highlights = useMemo(() => {
    // Generate some realistic beauty highlights if not present
    ...
    if (name.includes('cleanser')) {
      h.push('Gentle on skin')
      h.push('Removes impurities')
      h.push('Hydrating formula')
    } else if (name.includes('serum')) {
      h.push('Advanced skin repair')
      h.push('Brightens complexion')
      h.push('Dermatologist tested')
    } else if (name.includes('sunscreen')) {
      h.push('SPF 50+ protection')
      h.push('Non-greasy finish')
      h.push('Water resistant')
    } else {
      h.push('Premium quality')
      h.push('Dermatologist inspired')
      h.push('Suitable for all skin types')
    }
    
    h.push('Volume: ' + (product?.sizes?.[0] || '100ml'))
    h.push('12 Months Warranty')
    return h
  }, [product])
        ...
        <div className='sm:w-[260px]'>
          <div className='flex flex-col gap-2 rounded-2xl bg-slate-50 p-4 ring-1 ring-black/5'>
            ...
            <div className='text-sm text-slate-700'>
              Upto <span className='font-semibold text-slate-900'>{formatPrice(Math.round(price * 0.2))}</span> Off on Exchange
            </div>
            <div className='text-sm font-semibold text-emerald-700'>Bank Offer: {formatPrice(Math.round(price * 0.05))} off</div>
